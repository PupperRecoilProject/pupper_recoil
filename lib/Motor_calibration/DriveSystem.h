...
const uint8_t kNumDriveSystemDebugValues = 1 + 6 + 7*12; // 注意：這裡的 6 代表 IMU 數據，我們現在只有 3 個 (角速度)，未來需調整或重新計算
...
class DriveSystem {
 public:
  ...
  LSM6DSO& getIMU() { return imu; }
  void SetHomingThresholds(const std::array<float, 12>& thresholds);
 ...
 private:
  ...
  std::array<float, 12> homing_current_thresholds_; 
 ...
 public:
  ...
  DriveSystem(uint8_t imu_cs_pin = 8);

  // --- 新增啟用/禁用函式 ---
  void EnableSystem();  // <--- 新增這行
  void DisableSystem(); // <--- 新增這行
